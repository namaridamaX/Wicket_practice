...
public class MySession extends AbstractAuthenticatedWebSession{
    ...
    public void sign(String userName){
        //認証したユーザを変更する
        //セキュリティの配慮から、セッションを切り替える（古いセッションを使い回さない）
        replaceSession();
        this.userName = userName;
    }

    //認証したユーザの役割を返り値にするメソッド。Wicketはこの値を使って、ページを表示していいのかを判定する
    @Override
    public Roles getRoles(){
        //認証結果OK(userName = ユーザ名)だった場合は、誰もが"USER"権限を持っているとして返す
        //認証結果NG(userName = null)だった場合は、権限なしとして返す
        //wicket-auth-roles では、この権限の有無と種類でページを表示して良いか判断してくれる
        if(isSignedIn()){
